...
class New_Campaign_Frame:
    ...
    def button_submit(self):
        submit_button = Button(self.frame, text="Create", font=("Courrier", 25), bg='#000000',
                               fg='white', command=self.create_campaign)
        submit_button.grid(column=0, columnspan=2, row=2,sticky="ew")

    def labels(self):
        font = ("Courrier", 20)
        label_name = Label(self.frame, text="Campaign name : ", font=font, bg='#000000',fg='white')
    ...
    def print_player_list(self):
        frame_list_players = Frame(self.frame, bg="#000000")
        frame_list_players.grid(column=0, columnspan=2, row=1,sticky="ew")
        i = 0
        for player in self.player_list():
            self.check_box_dict[player.name] = IntVar()
            check_box = Checkbutton(frame_list_players, text = player.name+" "+player.pc_class, variable = self.check_box_dict[player.name], bg="#000000", fg="white")
